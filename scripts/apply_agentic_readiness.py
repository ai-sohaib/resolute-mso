...
# Business website. Public forms must not contain PHI or patient information.
"""
    ROBOTS.write_text(content, encoding="utf-8")


def ensure_llms() -> None:
    if LLMS.exists() and LLMS.stat().st_size > 100:
        return
    LLMS.write_text(
        "# Resolute MSO\n\nCanonical website: https://www.resolutemso.com/\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    required = [
        ROOT / "assets" / "js" / "agentic-readiness.js",
        ROBOTS,
        LLMS,
        INDEX,
    ]
    ...
    print("Agentic browsing readiness applied without changing visible website content.")
